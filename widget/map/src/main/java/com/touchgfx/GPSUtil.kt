...
object GPSUtil {
    ...
    fun getAddress(context: Context, latitude: Double, longitude: Double): Address? {
        return getAddress(context, Locale.getDefault(), latitude, longitude)
    }

    /**
     * 获取地址
     */
    fun getAddress(context: Context, locale: Locale, latitude: Double, longitude: Double): Address? {
        var address: Address? = null
        try {
            //Geocoder通过经纬度获取具体信息
            val gc = Geocoder(context, locale)
