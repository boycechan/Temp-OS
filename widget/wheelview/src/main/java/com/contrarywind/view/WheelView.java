...
import com.contrarywind.bean.NoneLabel;
...
public class WheelView extends View {
    ...
    private float density; // 屏幕密度比（0.75/1.0/1.5/2.0/3.0）
...
//        }
        if (!TextUtils.isEmpty(label) && isCenterLabel) {
            ...
            if (adapter.getItemsCount() > 0) {
                for (int i = adapter.getItemsCount() - 1; i < adapter.getItemsCount(); i--) {
                    Object item = adapter.getItem(i);
                    if (!(item instanceof NoneLabel)) {
                        text = item;
                        break;
                    }
                }
            }
            Object currentItem = adapter.getItem(getCurrentItem());
            if (!(currentItem instanceof NoneLabel)) {
                String contentText = getContentText(text);
                int contentTextWidth = getTextWidth(paintCenterText, contentText);
                int labelWidth = getTextWidth(paintCenterText, label);
                int prefixWidth = 0;
                if (!TextUtils.isEmpty(prefix)) {
                    prefixWidth = getTextWidth(paintCenterText, prefix);
                }
                int drawRightContentStart = contentTextWidth + prefixWidth + (measuredWidth - contentTextWidth - labelWidth - prefixWidth) / 2;
                int offset = (int) (density * 10);
                canvas.drawText(label, drawRightContentStart + CENTER_CONTENT_OFFSET + offset, centerY, paintCenterText);
                if (!TextUtils.isEmpty(prefix)) {
                    int prefixStart = drawRightContentStart - contentTextWidth - prefixWidth;
                    canvas.drawText(prefix, prefixStart - CENTER_CONTENT_OFFSET - offset, centerY, paintCenterText);
                }
            }
        }
        ...
        while (counter < itemsVisible) {
            ...
            if (angle > 90F || angle < -90F) {
                ...
                if (!TextUtils.isEmpty(label) && !TextUtils.isEmpty(getContentText(showText))) {
                    measuredText = getContentText(showText) + label;
                    if (!TextUtils.isEmpty(prefix)) {
                        measuredText = prefix + getContentText(showText) + label;
                    }
                } else {
                    measuredText = getContentText(showText);
                }
                ...
                reMeasureTextSize(measuredText);
                //计算开始绘制的位置
                measuredCenterContentStart(measuredText);
                measuredOutContentStart(measuredText);
...
//                }
                ...
                drawCenterContentStart = (int) ((measuredWidth - rect.width()) * 0.5) + prefixWidth;
                break;
            case Gravity.LEFT:
...
//                }
                ...
                drawOutContentStart = (int) ((measuredWidth - rect.width()) * 0.5) + prefixWidth;
                break;
            case Gravity.LEFT:
